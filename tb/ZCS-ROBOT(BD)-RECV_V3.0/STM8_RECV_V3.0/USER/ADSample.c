...
void ADSupply_Voltage_Scan(unsigned char CHAL)
{
    ...
    if (SUM_CNT[CHAL] >= 32)
    {
		...
		if((ACSupply_Volt_DD >= 300) && (STA_PAIR_CODE == 0))//大于1V时即认为有感应电压
		{
			CNT_TOUCH_SET++;
			if(CNT_TOUCH_SET >= 5)
			{
		...
		else if((ACSupply_Volt_DD <= 200) && (STA_PAIR_CODE == 1) && (Chanrg_Over == 0))  //20170221配对后未充满电取下
		{
			CNT_TOUCH_RE++;
			if(CNT_TOUCH_RE >= 31)  //3s
			{
				Touch_Coil_On = 0;
				CNT_TOUCH_RE = 0;
                //STA_PAIR_CODE = false;
		...
		else if((ACSupply_Volt_DD <= 200)  && (Chanrg_Over == 0))//20170221未配对未充满电取下
		{
			if(!STA_PAIR_CODE)
			{
				CNT_TOUCH_RE++;
		...
		else if((ACSupply_Volt_DD <= 200)  && (Chanrg_Over == 1)) //&& STA_PAIR_CODE
		{
			CNT_TOUCH_RE++;
