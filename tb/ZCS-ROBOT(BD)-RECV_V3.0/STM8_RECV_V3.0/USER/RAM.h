...
*******************************************************************************/
//发射机工作的状态
#define  C_MSIdle	0		// 初始化状态
#define  C_MSScan	1		// 查询
#define  C_MSPair	2		// 配对
#define  C_MSWork	30		// 供电
#define  C_MSProt	4       //异常状态

ramExtern unsigned long 		globetimer;//系统1ms滴答
ramExtern unsigned int 		PowOn2s;//系统1ms滴答
ramExtern unsigned char		CJ_SEND;
ramExtern unsigned char		Sys_Start;
//充电控制
...
ramExtern unsigned char		NOT_TACH;//异常延时
...
ramExtern unsigned char 		Queue_Full;
ramExtern unsigned char 		Queue_Full_P0_Changed;
ramExtern unsigned int 		add_master_add;
ramExtern unsigned char 		Pair_Succ;
ramExtern unsigned int 		Fail_Num;
ramExtern unsigned char		Recv_Msg;
//***********************************************
//计时
...
ramExtern unsigned char		 STA_PAIR_CODE;
//***********************************************
//加密算法数值
...
ramExtern unsigned char		over_input_vol; //输入线圈端过压
ramExtern unsigned char 		CNT_Power_Add; //暂时没使用
ramExtern unsigned char		CNT_Send_Time; //暂时没使用
ramExtern unsigned char 		Flag_Word; //暂时没使用
ramExtern unsigned char 		Flag_Right_Check; //暂时没使用
...
ramExtern unsigned char			  Flag_ADDRESS_SET_OK;
...
ramExtern unsigned char		  FLAG_CNT_TOUCH_SET;   //达到标量后表示接触
ramExtern unsigned int 		  CNT_TOUCH_RE;        //控制是否接触的计数标量不接触
ramExtern unsigned char		  FLAG_CNT_TOUCH_RE;   //达到标量后表示不接触
//***********************************************
//按键值
...
ramExtern unsigned char        FULL_DOWN_BETTERFLY;
//***********************************************
//风扇控制
...
//***********************************************
//收到主机的数据
ramExtern unsigned char         Touch_Coil_On;
