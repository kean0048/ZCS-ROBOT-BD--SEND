		...
		if(Flag_Eeprom_Ero == 0) 		//保密检测正常
        {
			...
			if(globetimer >= 200)
			{
				...
				if(Touch_Coil_On)
				{
					...
					if(Flag_Supply_Volt_OK == 2)//20170303 测试用
						CHECK_P0_SET();
							  ...
							  case C_MSPair:
								  ...
								  {	
									  ...
									  if(strncmp((char*)self_cpd,(char*)Address_Pair,5) == 0)
									  {
										  ...
										  //切换通道
										  if(Self_Num_Re == 1)
											CHECK_P_SET(RX_ADDRESS_1);
										  else if(Self_Num_Re == 2) 
											CHECK_P_SET(RX_ADDRESS_2);
										  else if(Self_Num_Re == 3)
											CHECK_P_SET(RX_ADDRESS_3);
										  else if(Self_Num_Re == 4)
											CHECK_P_SET(RX_ADDRESS_4);
										  else if(Self_Num_Re == 5)
											CHECK_P_SET(RX_ADDRESS_5);
				...
				{		
					...
					//20170222主机出故障且未充满电时进入以下程序
					/*if((Chanrg_Over == 0) && (Flag_Supply_Volt_OK == 1))//继电器本来就闭合 2017.01.17修改为未充满电时就拿走电池，必须初始化标示量
					{		
						CLEAR_P_SET();
						PF_ODR_ODR4 = 0;//继电器闭合
						Variable_Init_Second();
					...
					else if(PF_IDR_IDR4 == 1)//主机正常但充满后拿走需要闭合继电器
					{	
						PF_ODR_ODR4 = 0;//继电器闭合
						Flag_Supply_Volt_OK = 2;//标识进入过线圈脱离程序
						Variable_Init_Second();
