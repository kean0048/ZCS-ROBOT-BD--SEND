...
void Variable_Init(void)
{
    ...
    //FLAG_CNT_TOUCH_SET = 0;
    Bettery_Ok = 0; //电池通路
    Flag_Word = 0x11;       //离开发射线圈初始化值0x11
    FULL_DOWN_BETTERFLY = false;  //betterfly not full
	CNT_TOUCH_SET = 0;
	Pair_Succ = false;
	Queue_Full_P0_Changed = false;
	Step_ERR_Main = true; //主程序正常
	Flag_Right_Check = false;
	Touch_Coil_On = false;
	CNT_Send_Time = true;
	Step_Normal = false;
	Queue_Full = false;
...
void Variable_Init_Second()
{
    ...
    FULL_DOWN_BETTERFLY = false;  //betterfly not full
	Recv_Msg = false;   //配对需要转换标识
	Pair_Succ = false;	//未配对标识
	Step_Normal = false;	//指示灯闪烁
	Step_ERR_Main = true; //主程序正常
