...
void ADVoltage_Scan(unsigned char CHAL)
{
    ...
    if (SUM_CNT[CHAL] >= 32)
    {
        ...
        if((ACVolt_DD <= 2000) && (ACVolt_DD >= 1400) && (Flag_Supply_Volt_OK == 1) && FULL_DOWN_BETTERFLY)//电池电量小于5.3V认为电池电量耗尽 2017.01.18修改
		{
			PF_ODR_ODR4 = 0;//继电器闭合
            Variable_Init_Third();
		}
        //20170303 电池断开后处理方式
		/*if((ACVolt_DD <= 1400) && (ACCurr_DD <= 200) && (Bettery_Ok == 0))//20170222负载（电池）断路
		{
            if(Step_ERR_Main)
                PF_ODR_ODR4 = 1;//继电器打开
            Step_ERR_Main = false;
        ...
        else if((ACVolt_DD >= 1400) && (Bettery_Ok == 1))
        {
            Step_ERR_Main = true;
            Bettery_Ok = 0;
        }*/
...
void ADSupply_Voltage_Scan(unsigned char CHAL)
{
    ...
    if (SUM_CNT[CHAL] >= 32)
    {
		...
		else if(ACSupply_Volt_DD <= 10)  //20170221配对后未充满电取下
		{
			CNT_TOUCH_RE++;
